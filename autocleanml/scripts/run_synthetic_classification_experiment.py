...
NUMERIC_FEATURES = ["age", "tenure_years", "salary", "performance_score", "hours_per_week", "training_hours"]
CATEGORICAL_FEATURES = ["department", "education_level", "employment_type", "location"]
KEY_COLUMNS = ["department", "location"]
LABEL_COL = "attrition"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run synthetic classification cleaning and OPEX experiment."
    )
    ...
    try:
        ...
        synthetic = SyntheticDataGenerator(spark, config).generate_employee_attrition_dataset()
        ...
        ml_evaluator = SparkMLClassificationEvaluator(validation_folds=args.validation_folds)
        ...
        if args.log_dir:
            run_dir = log_run(
                Path(args.log_dir),
                "synthetic_employee_attrition",
                {
                    "metadata": {
                        "dataset": "synthetic_employee_attrition",
...
def run_manual_baseline(
    ...
    cleaned_df, actions = manual_clean_classification(dirty_df)
...
def manual_clean_classification(
    df: DataFrame,
    ...
    cleaned_df = cleaned_df.withColumn("salary", F.col("salary").cast("double"))
    actions.append({
        "issue": "schema_drift",
        "column": "salary",
    ...
    perf_missing = cleaned_df.filter(F.col("performance_score").isNull()).count()
    if perf_missing > 0:
        median = cleaned_df.approxQuantile("performance_score", [0.5], 0.01)[0]
        cleaned_df = cleaned_df.fillna({"performance_score": median})
        actions.append({
            "issue": "missingness",
    ...
    dept_missing = cleaned_df.filter(F.col("department").isNull()).count()
    if dept_missing > 0:
        cleaned_df = cleaned_df.fillna({"department": "unknown"})
        actions.append({
            "issue": "missingness",
            "column": "department",
            "strategy": "manual_constant_fill_unknown",
            "missing_count": dept_missing,
        })

    outlier_stats = manual_iqr_outlier_stats(cleaned_df, "salary", row_count)
    lower_bound = outlier_stats["lower_bound"]
    upper_bound = outlier_stats["upper_bound"]
    if lower_bound is not None and upper_bound is not None:
        cleaned_df = cleaned_df.withColumn(
            "salary",
            F.when(F.col("salary") < lower_bound, F.lit(lower_bound))
            .when(F.col("salary") > upper_bound, F.lit(upper_bound))
    ...
    skew_stats = manual_skew_stats(cleaned_df, "location")
    if skew_stats["severity"] == "high":
        cleaned_df = cleaned_df.repartition(4, F.col("location"))
        actions.append({
            "issue": "skew",
            "column": "location",
    ...
    if "label_noise" in issues:
        ...
        if noise_report.get("evaluated"):
            ...
            result["label_noise"] = {
                ...
                "suspected": detected,
                "accuracy": acc,
                "note": "RF confidence-based estimate; self-evaluation on training data.",
            }
            if acc is not None:
                numeric_accuracies.append(acc)
        else:
...
def build_report(
    *,
    ...
    return {
        "experiment": "synthetic_employee_attrition",
...
def build_manual_opex(
    ...
    source_lines = (
        inspect.getsourcelines(manual_clean_classification)[0]
    ...
    return {
        ...
        "seconds_per_1000_input_rows": (
            round(total_time / input_rows * 1000, 6) if input_rows else None
        ),
...
def print_summary(report: dict[str, Any]) -> None:
    ...
    print("\n=== Synthetic Employee Attrition Experiment ===")
    ...
    print(f"{'Condition':<30} {'Accuracy':>10} {'F1':>10} {'AUC':>10} {'Rows':>8}")
    print("-" * 72)
    for label, m in [
        ("Raw (dirty)", raw_ml),
        ...
        print(
            f"{label:<30} {str(m.get('accuracy')):>10} "
            f"{str(m.get('f1')):>10} {str(m.get('auc')):>10} "
    ...
    print(f"  mean_accuracy: {report['detection_accuracy'].get('mean_accuracy')}%")
